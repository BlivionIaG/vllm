...
class RDNA2Mxfp4MoEExperts(FusedMoEExpertsModular):
    ...
    def apply(
        ...
        w13_scales = (
            self._layer_ref.w13_weight_scale
            if self._layer_ref is not None
            else (a1q_scale if a1q_scale is not None else None)
        )
        if w13_scales is None:
            raise RuntimeError(
                "RDNA2Mxfp4MoEExperts: w13_weight_scale not available "
                "(self._layer_ref is None and a1q_scale is None). The "
                "kernel requires per-block UE8M0 weight scales for the "
                "MXFP4 weights."
            )
        ...
        w2_scales = (
            self._layer_ref.w2_weight_scale
            if self._layer_ref is not None
            else (a2_scale if a2_scale is not None else None)
        )
        if w2_scales is None:
            raise RuntimeError(
                "RDNA2Mxfp4MoEExperts: w2_weight_scale not available "
                "(self._layer_ref is None and a2_scale is None)."
            )
