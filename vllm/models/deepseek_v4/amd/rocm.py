...
class DeepseekV4ROCMAiterMLAAttention(DeepseekV4Attention):
    ...
    def _o_proj(self, o: torch.Tensor, positions: torch.Tensor) -> torch.Tensor:
        ...
        # (inverse RoPE + einsum) elsewhere. The preshuffled FP8 wo_b path
        # only activates when AITER is enabled (CDNA); on gfx10x the scales
        # stay None and this falls back to the plain wo_b GEMM.
        if on_gfx10x():
