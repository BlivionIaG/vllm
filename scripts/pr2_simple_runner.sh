#!/bin/bash
# PR #2 simplified matrix runner v6.
# - NO mtp (only num_speculative_tokens=0)
# - Concurrency: 1, 8, 32 only
# - Per-model timeout: qwen36=3000s, others=1500s
...
run_cell() {
  local workload_in=$1 workload_out=$2 conc=$3
  local num_prompts=$(get_num_prompts "$conc")
  local cell_name="${MODEL_ID}_tp${TP}_${KERNEL_NAME// /_}_SIMPLE_in${workload_in}_out${workload_out}_c${conc}"
  ...
  # SKIP if a SIMPLE cell with same (model, tp, kernel, wl_in, wl_out, conc) already has OK or TIMEOUT
  if [ -s "$CSV" ]; then
    local existing=$(awk -F, -v m="$MODEL_ID" -v t="$TP" -v k="$KERNEL_NAME" -v i="$workload_in" -v o="$workload_out" -v c="$conc" \
      'NR>1 && $2==m && $3==t && $4==k && $5==0 && $6==i && $7==o && $8==c && ($13=="OK" || $13=="TIMEOUT")' "$CSV")
    if [ -n "$existing" ]; then
      echo "[$(date +%H:%M:%S)] SKIP $cell_name (already have OK/TIMEOUT row)"
      return 0
    fi
  fi
  ...
  if ! wait_drained; then
    ...
    echo "$(date +%Y-%m-%dT%H:%M:%S),$MODEL_ID,$TP,$KERNEL_NAME,0,$workload_in,$workload_out,$conc,$num_prompts,,,,,SKIP,$cell_name" >> "$CSV"
    return 0
  fi

  echo "[$(date +%H:%M:%S)] START $cell_name (timeout=${CELL_TIMEOUT}s)"
  (
    trap 'echo "[runner-v6] TIMEOUT after ${CELL_TIMEOUT}s, killing bench"; exit 124' TERM
  ...
  echo "$(date +%Y-%m-%dT%H:%M:%S),$MODEL_ID,$TP,$KERNEL_NAME,0,$workload_in,$workload_out,$conc,$num_prompts,$out_tok_s,$total_tok_s,$ttft_p50,$status,$cell_name" >> "$CSV"
...
for wl in "${WORKLOADS[@]}"; do
  ...
  for conc in "${CONCS[@]}"; do
    run_cell $in_len $out_len $conc
  done
done

echo "[$(date +%H:%M:%S)] SIMPLE DONE for $MODEL_ID TP=$TP kernel=$KERNEL"